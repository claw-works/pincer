...
type Hub struct {
	mu           sync.RWMutex
	clients      map[string][]*Client // agentID → all active WS connections
	inbox        inboxBackend
	pubsub       PubSub
	ctx          context.Context
...
// IsOnlineWS reports whether the agent has at least one active WS connection.
func (h *Hub) IsOnlineWS(agentID string) bool {
	h.mu.RLock()
	defer h.mu.RUnlock()
	return len(h.clients[agentID]) > 0
}
...
func (h *Hub) Register(agentID string, conn *websocket.Conn) *Client {
	...
	h.mu.Lock()
	h.clients[agentID] = append(h.clients[agentID], c)
	isFirst := len(h.clients[agentID]) == 1
	h.mu.Unlock()
	go c.writePump()

	// Subscribe to Redis channel only on first connection for this agent.
	if isFirst {
...
func (h *Hub) Unregister(agentID string, conn *websocket.Conn) {
	h.mu.Lock()
	conns := h.clients[agentID]
	for i, c := range conns {
		if c.conn == conn {
			h.clients[agentID] = append(conns[:i], conns[i+1:]...)
			break
		}
	}
	remaining := len(h.clients[agentID])
	if remaining == 0 {
		delete(h.clients, agentID)
	}
	h.mu.Unlock()
	// Unsubscribe from Redis only when no connections remain.
	if remaining == 0 {
...
func (h *Hub) RegisterHuman(userID string, conn *websocket.Conn) *Client {
	...
	isFirst := len(h.clients[userID]) == 1
	h.mu.Unlock()
	go c.writePump()

	// Subscribe to Redis channel only on first connection for this user.
	if isFirst {
		_ = h.pubsub.Subscribe(h.ctx, func(msg Message) {
...
func (h *Hub) Send(to string, msg Message) {
	...
	if len(conns) > 0 {
		// Deliver to all active connections for this agent.
		for _, c := range conns {
			select {
			case c.send <- data:
			default:
				log.Printf("hub: send buffer full for agent %s conn, saving to inbox", to)
				h.inbox.SaveOffline(to, msg)
			}
		}
	} else {
		// Agent offline — try Redis pub/sub, then fall back to inbox.
...
// deliverLocal delivers a message received from Redis pub/sub to all locally-connected clients.
